...
export const PasteDetector: React.FC<PasteDetectorProps> = ({ 
  ...
  const { mode, parseStatus, enablePasteListener, disablePasteListener } = useClipboardStore();
          ...
          {isSupported ? (
            <div className="space-y-2">
              <p className="text-sm text-gray-600">
                粘贴检测器已启用。使用 <kbd className="px-2 py-1 text-xs font-semibold text-gray-800 bg-gray-100 border border-gray-200 rounded">Ctrl+V</kbd> 粘贴内容将自动触发剪贴板解析。
              </p>
              <div className="flex items-center space-x-2">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                <span className="text-xs text-green-600 font-medium">监听中</span>
              </div>
            </div>
          ) : (
            <div className="space-y-2">
